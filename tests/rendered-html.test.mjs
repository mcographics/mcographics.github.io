...
test("server-renders the Majestic Creations portfolio", async () => {
  ...
  assert.match(globalStyles, /\.featured-orbit-base\{[^}]*left:50%[^}]*bottom:-83px[^}]*width:480px[^}]*height:480px[^}]*padding:12px[^}]*overflow:visible/);
...
test("renders the complete Work Day with God product page", async () => {
  ...
  assert.match(html, /Work-Day-with-God-Setup-1\.4\.4\.exe/);
  assert.match(html, /Work-Day-with-God-Android-1\.0\.1\.apk/);
  assert.match(html, /aria-label="Quick download options"/);
  assert.match(html, /class="platform-download" href="https:\/\/github\.com\/mcographics\/WorkDaywithGod\/releases\/download\/v1\.4\.4\/Work-Day-with-God-Setup-1\.4\.4\.exe">Download EXE/);
