...
test("server-renders the Majestic Creations portfolio", async () => {
  ...
  assert.match(globalStyles, /\.hero-feature \.featured-release-card\.is-previous\{[^}]*opacity:1[^}]*filter:saturate\(\.66\) brightness\(\.72\)[^}]*rotateY\(30deg\) scale\(\.72\)/);
  ...
  assert.match(homepageSource, /setSelectedReleaseIndex\(\(current\) => \(current \+ 1\) % featuredReleases\.length\)/);
...
test("uses the current supplied Grace Seek banner and preserves its full curved layout", async () => {
  ...
  assert.match(globalStyles, /\.project-card\[data-project-title="Grace Seek"\] \.project-media\{aspect-ratio:16\/9\}/);
  assert.match(globalStyles, /\.project-card\[data-project-title="Grace Seek"\] \.project-media>img\{object-fit:contain!important;object-position:center!important;transform:none!important\}/);
