...
export async function generateMetadata({ params }: PageProps): Promise<Metadata> { const slug = (await params).slug; const tag = blogTags.find((item) => item.slug === slug); return tag ? { title: `${tag.name} topic | Majestic Creations Blog`, description: `Majestic Creations articles tagged ${tag.name}.`, alternates: { canonical: `/blog/tag/${tag.slug}/` } } : {}; }
