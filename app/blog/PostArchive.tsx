...
export default function PostArchive({ posts, heading = "All articles", startIndex = 1 }: { posts: BlogPost[]; heading?: string; startIndex?: number }) {
  return <div className="post-archive" aria-label={heading}>
    {posts.map((post, index) => <a className="post-card" href={`/blog/${post.slug}`} key={post.slug}>
      <span className="post-number">{String(index + startIndex).padStart(2, "0")}</span>
