...
export async function generateMetadata({ params }: PageProps): Promise<Metadata> { const slug = (await params).slug; const category = blogCategories.find((item) => item.slug === slug); return category ? { title: `${category.name} | Majestic Creations Blog`, description: `Majestic Creations articles filed under ${category.name}.`, alternates: { canonical: `/blog/category/${category.slug}/` } } : {}; }
