...
import DiscordCommunityLink from "../../community/DiscordCommunityLink";
...
export default async function ProjectPage({ params }: { params: Promise<{ slug: string }> }) {
  ...
  const releaseOptions = project.releaseDownloads ?? (project.release ? [{ platform: project.releasePlatform ?? "Android 7.0+", version: project.release.version, downloadUrl: project.release.downloadUrl, releaseUrl: project.release.releaseUrl, downloadLabel: "Download APK" }] : []);
  const schema = { "@context": "https://schema.org", "@type": "SoftwareApplication", name: project.title, url: projectUrl, image: project.image ? `${siteUrl}${project.image}` : undefined, description: project.description, applicationCategory: project.category, operatingSystem: project.platforms, softwareVersion: releaseOptions.map((release) => `${release.version} (${release.platform})`).join(", ") || undefined, downloadUrl: releaseOptions.map((release) => release.downloadUrl), isAccessibleForFree: true, author: { "@type": "Person", name: "Kenneth Salmon", url: `${siteUrl}/about/` }, publisher: { "@type": "Organization", name: "Majestic Creations", url: siteUrl }, sameAs: project.repository };
  return <main className="product-page generic-product" id="top">
    ...
    {releaseOptions.length ? <section className="product-downloads" id="downloads" aria-labelledby="downloads-heading"><header><p className="section-kicker">Official GitHub release</p><h2 id="downloads-heading">{project.releaseHeading ?? "Start breaking the chains."}</h2><p>{project.releaseDescription ?? `ChainBreaker ${releaseOptions[0].version} is available as a direct APK for Android 7.0 and newer. The release is local-first, account-free, and intended for private use on the reader’s own device.`}</p></header><div className="download-grid">{releaseOptions.map((release) => <article key={`${release.platform}-${release.version}`}><div><span className="download-platform">{release.platform}</span><strong>v{release.version}</strong><small>{project.releaseSmallText ?? "Release artifact · GitHub distribution"}</small></div><p>Download the official artifact from GitHub Releases. Review the release notes and verify the source before installing.</p><a className="button primary" href={release.downloadUrl}>{release.downloadLabel ?? (release.platform.toLowerCase().includes("android") ? "Download APK" : "Download") } <span>↓</span></a><a href={release.releaseUrl} target="_blank" rel="noreferrer">Release notes ↗</a></article>)}<article><div><span className="download-platform">Source</span><strong>GitHub</strong><small>Public repository and history</small></div><p>Inspect the application source, bundled content, platform projects, and release history in the public repository.</p><a className="button ghost" href={project.repository} target="_blank" rel="noreferrer">Open repository <span>↗</span></a><a href={releaseOptions[0].releaseUrl} target="_blank" rel="noreferrer">View release ↗</a></article></div></section> : null}
    ...
    {!project.private && !project.specialAccess ? <DiscordCommunityLink slug={project.slug} title={project.title} /> : null}
