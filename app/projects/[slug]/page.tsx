...
export default async function ProjectPage({ params }: { params: Promise<{ slug: string }> }) {
  ...
  const requestHref = project.slug === "fierolink-gt" ? "mailto:majesticcreationsottawa@outlook.com?subject=FieroLink%20GT%20access%20request" : project.requestHref;
  ...
  return <main className="product-page generic-product" id="top">
    ...
    {project.release ? <section className="product-downloads" id="downloads" aria-labelledby="downloads-heading"><header><p className="section-kicker">Official Android release</p><h2 id="downloads-heading">Start breaking the chains.</h2><p>ChainBreaker {project.release.version} is available as a direct APK for Android 7.0 and newer. The release is local-first, account-free, and intended for private use on the reader’s own device.</p></header><div className="download-grid"><article><div><span className="download-platform">Android 7.0+</span><strong>v{project.release.version}</strong><small>Signed release APK · local-first</small></div><p>Download the official APK from GitHub Releases. Review the release notes and verify the source before installing.</p><a className="button primary" href={project.release.downloadUrl}>Download APK <span>↓</span></a><a href={project.release.releaseUrl} target="_blank" rel="noreferrer">Release notes ↗</a></article><article><div><span className="download-platform">Source</span><strong>GitHub</strong><small>Public repository and history</small></div><p>Inspect the application source, bundled content, Android project, and release history in the public repository.</p><a className="button ghost" href={project.repository} target="_blank" rel="noreferrer">Open repository <span>↗</span></a><a href={project.release.releaseUrl} target="_blank" rel="noreferrer">View release ↗</a></article></div></section> : null}
