                ...
                {group.projects.map((project) => (
                  <article className="project-card" key={project.title} data-project-title={project.title} data-categories={projectCategories(project).join(" ")} style={{ "--project-color": project.color } as React.CSSProperties}>
                    ...
                    <div className="project-info">
                      <div className="project-heading"><span><small>{project.type}</small><h3>{project.title}</h3></span><div className="project-actions">{"releaseVersions" in project && project.releaseVersions?.length ? project.releaseVersions.length === 1 && /\.apk$/i.test(project.releaseVersions[0].url) ? <a className="project-download" href={project.releaseVersions[0].url} target="_blank" rel="noreferrer" aria-label={`Download ${project.title}`}>Download</a> : <button type="button" className="project-download" onClick={() => setReleaseProject({ title: project.title, versions: project.releaseVersions! })} aria-label={`Choose a version of ${project.title}`}>Choose Version</button> : "downloadUrl" in project && project.downloadUrl ? <a className="project-download" href={project.downloadUrl} target="_blank" rel="noreferrer" aria-label={`${project.downloadLabel ?? "Download"} for ${project.title}`}>Download</a> : null}{"specialAccess" in project && project.specialAccess ? <>{project.private ? <span className="project-lock private">Private</span> : null}<a className="project-lock special" href={project.requestHref ?? "/about#connect"} aria-label={`Request access to ${project.title}`}>{project.requestLabel ?? "Request Required"}</a></> : project.link ? <><span className="project-lock public">Public</span><a href={project.link} target="_blank" rel="noreferrer" aria-label={`Open ${project.title}`}><img src="/brand/github-invertocat-white.png" alt="" /></a></> : <span className={`project-lock${project.private ? " private" : ""}`}>{project.private ? "Private" : "Studio project"}</span>}</div></div>
